...
def export_to_psd(original_ortho_path, png_layer_paths, output_dir):
    """
    img2psd.py 外部スクリプトを呼び出してPSDファイルを生成する。
    WindowsのCP932(Shift-JIS)エンコーディングエラーに対応する。
    """
    ...
    if not os.path.exists(img2psd_script_path):
        raise FileNotFoundError(
            f"img2psd.py が見つかりません。"
            f"次の場所を確認してください: {img2psd_script_path}"
        )

    all_image_paths = [os.path.abspath(original_ortho_path)] + \
                      [os.path.abspath(p) for p in png_layer_paths]

    command = [sys.executable, img2psd_script_path] + all_image_paths

    print(f"外部スクリプトを呼び出します: {img2psd_script_path}")
    ...
    try:
        ...
        try:
            ...
            print("UTF-8デコード失敗。CP932 (Shift-JIS) で再試行します。")
        ...
        if not output_lines:
            if stderr_text:
                print(f"img2psd.py がエラーを返しました: {stderr_text}")
        ...
        if last_line.startswith(prefix):
            output_path = last_line[len(prefix):].strip()
...
class ImageProcessor(QObject):
    ...
    def _perform_coarse_alignment(self, ortho_img, detail_img):
        ...
        detail_small = cv2.resize(
            detail_img, (RESIZE_WIDTH_COARSE, int(detail_h * scale_d)),
            interpolation=cv2.INTER_AREA
        )
        ...
        if des_o_s is None or des_d_s is None:
            raise ValueError("[Coarse] 特徴量計算失敗")
        ...
        if len(good_matches) < 10:
            raise ValueError("[Coarse] 初期対応点が見つかりません。")

        src_pts = np.float32(
            [kp_d_s[m.queryIdx].pt for m in good_matches]
        ).reshape(-1, 1, 2) / scale_d
        dst_pts = np.float32(
    ...
    def _get_line_intersection(self, p1, p2, p3, p4):
        ...
        u = -((p2[0] - p1[0]) * (p3[1] - p1[1]) -
              (p2[1] - p1[1]) * (p3[0] - p1[0])) / d
        if t >= 0 and 0 <= u <= 1:
            return p1 + t * (p2 - p1)
        return None

    def _get_bisector_intersection(
            self, p_curr, p_prev, p_next, warped_border_lines):
        ...
        if np.linalg.norm(bisector_vec) < 1e-6:
            return None
        p_far = p_curr + bisector_vec * 1e6
        for i, (line_start, line_end) in enumerate(warped_border_lines):
            intersect_pt = self._get_line_intersection(
                p_curr, p_far, line_start, line_end
            )
            if intersect_pt is not None:
                return (QPointF(*intersect_pt), i)
        return None
    ...
    def _generate_extension_polygons(self, hull_points, warped_corners):
        ...
        if num_hull_pts < 2:
            return polygons
        warped_border_lines = [
            (warped_corners[i], warped_corners[(i + 1) % 4]) for i in range(4)
        ]
        for i in range(num_hull_pts):
            h1, h2 = hull_points[i], hull_points[(i + 1) % num_hull_pts]
            res1 = self._get_bisector_intersection(
                h1, hull_points[i - 1], h2, warped_border_lines
            )
            if not res1:
                continue
            i1, edge_idx1 = res1
            res2 = self._get_bisector_intersection(
                h2, h1, hull_points[(i + 2) % num_hull_pts],
                warped_border_lines
            )
            if not res2:
                continue
            ...
            unique_path_nodes = [
                p for j, p in enumerate(path_nodes)
                if j == 0 or path_nodes[j - 1].toPoint() != p.toPoint()
            ]
            polygons.append(
                QPolygonF([QPointF(*h2), QPointF(*h1)] + unique_path_nodes)
            )
        return polygons
    ...
    def create_and_save_final_image(self, data, selected_polygons):
        ...
        ortho_img, detail_img, M = (
            data["ortho_image"], data["detail_image"], data["homography_matrix"]
        )
        ortho_h, ortho_w = ortho_img.shape[:2]
        if detail_img.shape[2] == 3:
            detail_img_bgra = cv2.cvtColor(detail_img, cv2.COLOR_BGR2BGRA)
        else:
            detail_img_bgra = detail_img.copy()
        warped_detail_bgra = cv2.warpPerspective(
            detail_img_bgra, M, (ortho_w, ortho_h), flags=cv2.INTER_LINEAR,
            borderMode=cv2.BORDER_TRANSPARENT
        )
        final_mask = np.zeros((ortho_h, ortho_w), dtype=np.uint8)
        for poly in selected_polygons:
            pts = np.array(
                [[p.x(), p.y()] for p in poly.toList()], dtype=np.int32
            )
            cv2.fillPoly(final_mask, [pts], 255)
        warped_detail_bgra[:, :, 3] = cv2.bitwise_and(
            warped_detail_bgra[:, :, 3], final_mask
        )
...
class PhotoViewer(QGraphicsView):
    ...
    def __init__(self, scene):
        ...
        self.selection_rect_item.setPen(
            QPen(QColor(0, 120, 215, 200), 2, Qt.DashLine)
        )
    ...
    def mousePressEvent(self, event):
        ...
        if self.current_mode == self.MODE_SELECTION and \
           event.button() == Qt.LeftButton:
            self._is_selecting = True
            if self.selection_rect_item.scene() is None:
                self.scene().addItem(self.selection_rect_item)
            self._selection_start_pos = self.mapToScene(
                event.position().toPoint()
            )
            self.selection_rect_item.setRect(
                QRectF(self._selection_start_pos, QSize(0, 0))
            )
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.RightButton and self._is_panning:
            ...
            if self.selection_rect_item.rect().width() > 5:
...
class CandidateDialog(QDialog):
    ...
    def accept(self):
        if self.list_widget.selectedItems():
            self.selected_path = (
                self.list_widget.selectedItems()[0].data(Qt.UserRole)
            )
            super().accept()
...
class MainWindow(QMainWindow):
    ...
    def setup_scene(self):
        ...
        if ortho_cv_img is None:
            self.on_processing_error("OpenCVでのオルソ画像の読み込みに失敗しました。")
            return
        pixmap = self._cv_to_qpixmap(ortho_cv_img)
        if pixmap.isNull():
            self.on_processing_error("オルソ画像の表示に失敗")
            return
    ...
    def on_region_selected(self, rect):
        ...
        self.status_label.setText(
            "領域を選択しました。「② この領域で候補を検索」ボタンを押してください。"
        )

    def on_find_button_clicked(self):
        self.view.clear_selection()
        candidates = self.find_candidate_images(
            QPolygonF(self.current_selection_rect)
        )
        if not candidates:
            QMessageBox.information(
                self, "情報", "選択領域にマッチする画像が見つかりませんでした。"
            )
            self.reset_ui_state()
            return
        ...
        if dialog.exec():
            ...
            self.start_processing_signal.emit(
                self.current_ortho_path, dialog.selected_path
            )
        else:
            self.reset_ui_state()
    ...
    def on_processing_finished(self, data):
        ...
        warped_pixmap = self._cv_to_qpixmap(
            cv2.warpPerspective(
                data["detail_image"], data["homography_matrix"],
                (data["ortho_image"].shape[1], data["ortho_image"].shape[0])
            )
        )
        ...
        colors = [
            QColor(255, 0, 0, 80), QColor(0, 0, 255, 80),
            QColor(255, 255, 0, 80), QColor(255, 0, 255, 80)
        ]
        for i, poly in enumerate(data['extension_polygons']):
            ...
            item.mousePressEvent = lambda event, idx=i: self.toggle_selection(
                idx
            )
        ...
        self.status_label.setText(
            "③ 合成に含めたい拡張領域をクリックし、「選択を確定して合成」を押してください。"
        )
    ...
    def save_result(self):
        final_polygons = [
            QPolygonF([QPointF(*p) for p in self.data["hull_points"]])
        ]
        final_polygons.extend(
            self.data['extension_polygons'][i]
            for i, selected in enumerate(self.selected_states) if selected
        )
        self.confirm_button.setEnabled(False)
    ...
    def on_update_finished(
            self, new_ortho_path, original_png_path, blended_image):
        ...
        if new_pixmap.isNull():
            self.on_processing_error("更新されたオルソ画像の表示に失敗しました。")
            return
    ...
    def find_candidate_images(self, selection_polygon):
        ...
        for path in self.detail_paths:
            cache = load_cache(self.initial_ortho_path, path)
            if cache and 'warped_corners' in cache:
                detail_poly = QPolygonF(
                    [QPointF(*p) for p in cache['warped_corners']]
                )
                if detail_poly.intersects(selection_polygon):
                    candidates_with_thumbs.append(
                        (path, QPixmap(path).scaled(
    ...
    def _cv_to_qpixmap(self, cv_img):
        ...
        return QPixmap.fromImage(
            QImage(cv_img.data, w, h, bytes_per_line, QImage.Format_BGR888)
        )
    ...
    def export_project(self):
        ...
        try:
            ...
            QMessageBox.information(
                self, "成功", f"PSDファイルを保存しました:\n{path}"
            )
        except Exception as e:
            self.on_processing_error(f"PSDエクスポート失敗: {e}")
        finally:
            self.reset_ui_state()
